...
Otherwise, SMTC for windows ( and future providers ) will be the active now-playing provider, providing metadata for whatever the platofrm reports as the currently playing media (Spotify, browsers, media players, etc.) on the host machine.

Other platform implementations for Linux and Mac will be implemented in due course.

Sendspin will provide album art referencs for the playing track.

Providers such as SMTC can provide poorly matched video thumbnails, therefore for the platform providers, the track metadata is normalised to remove common cruft ( such as HD Official Video ) and submitted to [MusicBrainz](https://musicbrainz.org/) / [Cover Art Archive](https://coverartarchive.org/) to resolve the album artework.
...
The MusicBrainz / Cover Art Archive response will be several seconds after the intial display of the track title.
...
The dialog has a top section showing the current playing album art, track and albym title. It is then divided into three sections: **Gradient**, **Track Text**, and **Album Art**.
