...
Nanoleaf release new hardware and firmware which may be incompatible. If you have an issue with a Nanoleaf device then reach out on discord. 
...
It is necessary to obtain an authentication token from your Nanoleaf device when first configuring in LedFx

**Press and hold** the power button on your Nanoleaf controller for 5-7 seconds until the LEDs flash
 
Within 30 seconds, press the GET TOKEN button in the LedFx device config. A token should appear in the text box. This token will be saved with your LedFx config and will only need replacing should you reset your Nanoleaf device Firmware.
