...
class _DummyDevice:
    ...
    def clear_virtual_segments(self, virtual_id):
        self._segments = [
            s for s in self._segments if s[0] != virtual_id
        ]
...
def _make_virtual(ledfx, virtual_id, name, segments, is_device=False, auto_generated=False):
...
class TestDeviceDeletionDestroysUserVirtual:
    ...
    async def test_user_virtual_destroyed_after_device_removal(self):
        ...
        # Import and call the real remove_from_virtuals
        from ledfx.devices import Device
