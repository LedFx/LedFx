...
class TestAssetsAPIThumbnail:
    ...
    def test_thumbnail_force_refresh(self, sample_png_bytes):
        ...
        # Get thumbnail first time (will be cached)
        resp1 = requests.post(
            ASSETS_THUMBNAIL_API_URL,
        ...
        # Get thumbnail second time (should come from cache)
        resp2 = requests.post(
            ASSETS_THUMBNAIL_API_URL,
            json={"path": "test_force_refresh.png", "size": 64},
            headers={"Content-Type": "application/json"},
        ...
        assert resp2.content == content1  # Should be identical (from cache)

        # Get thumbnail with force_refresh=true (should regenerate)
        resp3 = requests.post(
            ASSETS_THUMBNAIL_API_URL,
            json={
        ...
        # Content should be the same (same image, same params)
        # but it was regenerated bypassing cache
        ...
        # Should be the same as the regenerated version
        assert resp4.content == resp3.content
