...
class WavelengthAudioEffect(AudioReactiveEffect, GradientEffect):
    ...
    CONFIG_SCHEMA = vol.Schema({
        ...
        vol.Optional('reduce', default = True): bool,
    })

    def audio_data_updated(self, data):
        if self._config['reduce']:
            reduce_amount = 2
        else:
            reduce_amount = 1
        # Grab the filtered and interpolated melbank data
        y = data.interpolated_melbank(self.pixel_count // reduce_amount, filtered = False)
        filtered_y = data.interpolated_melbank(self.pixel_count // reduce_amount, filtered = True)

        # Grab the filtered difference between the filtered melbank and the
        # raw melbank.
        r = data.get_filter(
            filter_key = "filtered_difference",
            filter_size = self.pixel_count // reduce_amount,
            alpha_decay = 0.2,
