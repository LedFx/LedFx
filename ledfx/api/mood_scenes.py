...
class MoodScenesEndpoint(RestEndpoint):
    ...
    async def post(self, request: web.Request) -> web.Response:
        ...
        try:
            ...
            if "type" not in data or "key" not in data or "scene_id" not in data:
                return await self.invalid_request(
            ...
            return await self.request_success(
                "success",
                f"Mood scene mapping added: {key} -> {scene_id}"
            )
            
        except Exception as e:
            _LOGGER.error(f"Error adding mood scene mapping: {e}", exc_info=True)
            return await self.internal_error(str(e))
    ...
    async def delete(self, request: web.Request) -> web.Response:
        ...
        try:
            ...
            return await self.request_success(
                "success",
                f"Mood scene mapping removed: {key}"
            )
            
        except Exception as e:
            _LOGGER.error(f"Error removing mood scene mapping: {e}", exc_info=True)
            return await self.internal_error(str(e))
