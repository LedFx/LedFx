...
class MoodDetector:
    ...
    def __init__(self, audio_source: Any, config: Optional[Dict[str, Any]] = None) -> None:
        """
        ...
        try:
            ...
            if sample_rate <= 0:
                _LOGGER.warning(f"Invalid sample_rate: {sample_rate}, using default 60")
                sample_rate = 60
        except (AttributeError, KeyError):
            _LOGGER.warning("Could not get sample_rate from audio source, using default 60")
            sample_rate = 60
        
        history_size = max(10, int(self._config["history_length"] * sample_rate))
        ...
        self._feature_extraction_interval = 0.1  # Extract features max 10x per second
        ...
        self._use_librosa = (
            self._config.get("use_librosa", False) and 
            LIBROSA_MODULE_AVAILABLE and 
            is_librosa_available()
        )
        ...
        if self._use_librosa:
            try:
                ...
                if self._librosa_extractor:
                    _LOGGER.info("Librosa integration enabled for enhanced mood detection")
                else:
                    _LOGGER.warning("Librosa requested but extractor creation failed")
        ...
        if self._use_librosa and self._librosa_extractor:
            try:
                ...
                if audio_sample is not None and len(audio_sample) > 0:
                    self._librosa_extractor.audio_buffer.add_sample(audio_sample)
            except (AttributeError, TypeError) as e:
                _LOGGER.debug(f"Error adding audio to librosa buffer: {e}")
            except Exception as e:
                _LOGGER.warning(f"Unexpected error adding audio to librosa buffer: {e}")
                ...
                try:
                    ...
                    if features:
                        ...
                        if features.get('rhythm_regularity') is not None:
                            rhythm_regularity = float(np.clip(features['rhythm_regularity'], 0.0, 1.0))
                        
                        # Harmonic ratio (melodic vs percussive)
                        if features.get('harmonic_ratio') is not None:
                            harmonic_ratio = float(np.clip(features['harmonic_ratio'], 0.0, 1.0))
                        
                        # Complexity (based on spectral bandwidth and MFCC variance)
                        if features.get('spectral_bandwidth_norm') is not None:
                            bandwidth = float(features['spectral_bandwidth_norm'])
                            if features.get('mfcc_std') is not None:
                                mfcc_std = np.mean(features['mfcc_std']) if isinstance(features['mfcc_std'], np.ndarray) else 0.0
                                complexity = float(np.clip((bandwidth * 0.6 + mfcc_std * 0.4), 0.0, 1.0))
                            else:
                                complexity = float(np.clip(bandwidth, 0.0, 1.0))
                except (TypeError, AttributeError, ValueError) as e:
                    _LOGGER.debug(f"Error extracting additional mood metrics: {e}")
            
            # Update current mood atomically
            self._current_mood.update({
                "energy": float(np.clip(self._energy_filter.value, 0.0, 1.0)),
                "valence": float(np.clip(self._valence_filter.value, 0.0, 1.0)),
                "intensity": float(np.clip(intensity, 0.0, 1.0)),
                "brightness": float(np.clip(brightness, 0.0, 1.0)),
                "beat_strength": float(np.clip(beat_strength, 0.0, 1.0)),
                "spectral_warmth": float(np.clip(spectral_warmth, 0.0, 1.0)),
                "rhythm_regularity": float(np.clip(rhythm_regularity, 0.0, 1.0)),
                    ...
                    if features:
                        ...
                        if features.get('onset_rate_norm') is not None:
                            try:
                                ...
                                energy_raw = (energy_raw * 0.7 + onset_rate * 0.3)
                            except (TypeError, ValueError):
                                pass
                        
                        if features.get('onset_strength_norm') is not None:
                            try:
                                onset_strength = float(features['onset_strength_norm'])
                                onset_strength = np.clip(onset_strength, 0.0, 1.0)
                                energy_raw = (energy_raw * 0.8 + onset_strength * 0.2)
                            except (TypeError, ValueError):
                                pass
                except (TypeError, ValueError, AttributeError) as e:
                    _LOGGER.debug(f"Error using librosa features for energy: {e}")
    ...
    def _calculate_valence(self) -> float:
        ...
        try:
            ...
            valence = (mids_power * 0.4 + high_power * 0.4 + (1.0 - bass_power) * 0.2)
                        ...
                        if features.get('chroma') is not None:
                            try:
                                ...
                                if len(chroma) >= 12:
                                    ...
                                    bright_chroma = float(np.mean(chroma[6:]))  # Upper half
                                    bright_chroma = np.clip(bright_chroma, 0.0, 1.0)
                                    valence = (valence * 0.7 + bright_chroma * 0.3)
                            except (TypeError, ValueError, IndexError):
                                _LOGGER.debug("Error processing chroma features")
                except (TypeError, AttributeError) as e:
                    _LOGGER.debug(f"Error using librosa features for valence: {e}")
                ...
                try:
                    ...
                    if features:
                        ...
                        if features.get('spectral_centroid_norm') is not None:
                            try:
                                centroid = float(features['spectral_centroid_norm'])
                                centroid = np.clip(centroid, 0.0, 1.0)
                                brightness = (brightness * 0.5 + centroid * 0.5)  # Higher weight for centroid
                            except (TypeError, ValueError):
                                pass
                        ...
                        if features.get('brightness_contrast') is not None:
                            try:
                                contrast = float(features['brightness_contrast'])
                                # Normalize contrast to 0-1 range (empirical scaling)
                                contrast_norm = np.clip(contrast / 10.0, 0.0, 1.0)
                                brightness = (brightness * 0.7 + contrast_norm * 0.3)
                            except (TypeError, ValueError):
                                pass
                        ...
                        if features.get('spectral_rolloff_norm') is not None:
                            try:
                                rolloff = float(features['spectral_rolloff_norm'])
                                rolloff = np.clip(rolloff, 0.0, 1.0)
                                brightness = (brightness * 0.8 + rolloff * 0.2)
                            except (TypeError, ValueError):
                                pass
                except (TypeError, AttributeError) as e:
                    _LOGGER.debug(f"Error using librosa features for brightness: {e}")
    ...
    def _calculate_beat_strength(self) -> float:
        ...
        try:
            ...
            _LOGGER.debug(f"Error calculating beat strength: {e}", exc_info=False)
            return 0.5
        ...
        if include_genre:
            try:
                ...
                if music_styles and isinstance(music_styles, dict):
                    ...
                    dominant_genre = max(music_styles.items(), key=lambda x: x[1] if isinstance(x[1], (int, float)) else 0)
                    if dominant_genre[1] > 0.15:  # Minimum confidence threshold
        ...
        if self._use_librosa and self._librosa_extractor:
            ...
            if features:
                ...
                if features.get('spectral_rolloff_norm'):
                    metrics['spectral_rolloff'] = features['spectral_rolloff_norm']
                if features.get('spectral_centroid_norm'):
                    metrics['spectral_centroid'] = features['spectral_centroid_norm']
                ...
                try:
                    ...
                    if style_scores:
                        ...
                        if style_scores:
                            ...
                            try:
                                sub_genre_scores = classify_sub_genre(features, dominant_style[0])
                                if sub_genre_scores:
                                    metrics['sub_genres'] = sub_genre_scores
                                    # Get dominant sub-genre
                                    if sub_genre_scores:
                                        dominant_sub = max(sub_genre_scores.items(), key=lambda x: x[1])
                                        if dominant_sub[1] > 0.2:  # Minimum confidence
                                            metrics['dominant_sub_genre'] = dominant_sub[0]
                                            metrics['sub_genre_confidence'] = dominant_sub[1]
                            except Exception as e:
                                _LOGGER.debug(f"Error classifying sub-genre: {e}")
