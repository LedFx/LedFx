...
class MQTT_HASS(Integration):
    ...
    def on_message(self, client, userdata, msg):
        ...
        if msg.topic.endswith("/set"):
            ...
            if virtualid == "ledfxsceneselect":
                ...
                for virtual in self._ledfx.virtuals.values():
                    if virtual.id not in scene["virtuals"].keys():
                        _LOGGER.warning(
                            (
                                "virtual with id {} has no data in scene {}"
                            ).format(virtual.id, scene_id)
                        )
                        continue
            ...
            client.publish(
                f"{self._config['topic']}/light/{virtualid}/state", msg.payload
            )
