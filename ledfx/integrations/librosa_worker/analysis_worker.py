...
class AudioAnalyzer:
    ...
    def __init__(self):
        ...
        self._prev_z_energy = (
            0.0  # Store previous z-score for energy change detection
        )

        # Tunable knobs (could come from config later)
        self._section_threshold = (
            0.4  # Lower = more sensitive (was 1.0, too high)
        )
        self._section_min_gap_steps = (
            2  # with 2 s analysis step → min ~4 s between sections (was 8s)
        )
    ...
    def process_audio_block(self, block: np.ndarray):
        ...
        if (
            self.buffer.size
            >= int(self.sample_rate * MIN_SECONDS_BEFORE_ANALYSIS)
            and current_time - self.last_analysis_time
            >= PROCESS_PERIOD_SECONDS
        ):
    ...
    def _analyze_and_emit(self):
        ...
        if self._feat_mean is None or self._feat_prev is None:
            ...
            self._energy_var = beta * self._energy_var + (1.0 - beta) * (
                delta_e**2
            )
            ...
            self._onset_mean_var = beta * self._onset_mean_var + (
                1.0 - beta
            ) * (delta_o**2)
            ...
            self._feat_mean = (
                alpha * self._feat_mean + (1.0 - alpha) * curr_feat
            )
            ...
            z_brightness = (
                centroid - 2000.0
            ) / 1000.0  # Crude normalization around 2kHz
...
if __name__ == "__main__":
    ...
    try:
        ...
        sys.stderr.write(
            "[Worker] debugpy listening on port 5679 (ready for attach)\n"
        )
        sys.stderr.flush()
    except ImportError:
