...
class LibrosaAudioBuffer:
    ...
    def __init__(self, sample_rate: int = 30000, buffer_duration: float = 3.0):
        ...
        if buffer_duration <= 0:
            raise ValueError(f"Invalid buffer_duration: {buffer_duration} (must be > 0)")
    ...
    def get_fill_ratio(self) -> float:
        ...
        """
        with self.lock:
            return min(1.0, len(self.buffer) / self.buffer_size) if self.buffer_size > 0 else 0.0
    ...
    def extract_features(self) -> Optional[FeatureDict]:
        ...
        try:
            ...
            try:
                ...
                key, mode = librosa.harmonic.key(y=y_harmonic, sr=self.target_sample_rate)
                features['key'] = str(key) if key is not None else None
                features['mode'] = str(mode) if mode is not None else None  # 'major' or 'minor'
                features['mode_numeric'] = 1.0 if mode == 'major' else (0.0 if mode == 'minor' else 0.5)
            ...
            try:
                ...
                features['chroma'] = np.mean(chroma, axis=1)  # Average across time
            ...
            try:
                ...
                features['brightness_contrast'] = np.mean(spectral_contrast[-3:])
            except Exception as e:
            ...
            try:
                ...
                features['spectral_rolloff_norm'] = np.clip(
                    features['spectral_rolloff'] / (self.target_sample_rate / 2),
                    0.0, 1.0
                )
            except Exception as e:
            ...
            try:
                ...
                features['mfcc_timbre'] = np.mean(mfcc[1:6], axis=1)  # Timbre coefficients
            except Exception as e:
            ...
            try:
                ...
                features['spectral_centroid_norm'] = np.clip(
                    features['spectral_centroid'] / (self.target_sample_rate / 2),
                    0.0, 1.0
                )
            ...
            try:
                ...
                features['spectral_bandwidth_norm'] = np.clip(
                    features['spectral_bandwidth'] / (self.target_sample_rate / 2),
                    0.0, 1.0
                )
            except Exception as e:
            ...
            try:
                ...
                if len(onset_frames) > 0:
                    ...
                    features['onset_rate'] = len(onset_frames) / duration if duration > 0 else 0.0
                    # Normalize to 0-1 (assuming max ~10 onsets/sec)
                    features['onset_rate_norm'] = np.clip(features['onset_rate'] / 10.0, 0.0, 1.0)
                    ...
                    features['onset_strength_norm'] = np.clip(features['onset_strength'], 0.0, 1.0)
                else:
            ...
            try:
                ...
                features['tonnetz'] = np.mean(tonnetz, axis=1)  # 6-dimensional feature
                features['tonnetz_stability'] = float(1.0 - np.std(tonnetz))  # Higher = more stable harmony
            except Exception as e:
            ...
            try:
                ...
                y_harmonic, y_percussive = librosa.effects.hpss(audio_resampled)
            ...
            try:
                if features.get('tempo') is not None and features.get('beat_times') is not None:
                    beat_times = features['beat_times']
                    if len(beat_times) > 1:
                        # Calculate inter-beat intervals
                        intervals = np.diff(beat_times)
                        # Regularity: lower std = more regular rhythm
                        features['rhythm_regularity'] = float(1.0 - np.clip(np.std(intervals) / np.mean(intervals), 0.0, 1.0))
                        # Tempo stability
                        expected_interval = 60.0 / features['tempo'] if features['tempo'] > 0 else 0.5
                        tempo_stability = 1.0 - np.clip(np.abs(np.mean(intervals) - expected_interval) / expected_interval, 0.0, 1.0)
            ...
            _LOGGER.warning(f"Error in librosa feature extraction: {e}", exc_info=True)
...
def classify_music_style(features: FeatureDict) -> Dict[str, float]:
    ...
    try:
        ...
        tempo = float(features.get('tempo', 120.0)) if features.get('tempo') is not None else 120.0
        spectral_centroid = float(features.get('spectral_centroid_norm', 0.5)) if features.get('spectral_centroid_norm') is not None else 0.5
        zero_crossing = float(features.get('zero_crossing_rate', 0.05)) if features.get('zero_crossing_rate') is not None else 0.05
        harmonic_ratio = float(features.get('harmonic_ratio', 0.5)) if features.get('harmonic_ratio') is not None else 0.5
        percussive_ratio = float(features.get('percussive_ratio', 0.5)) if features.get('percussive_ratio') is not None else 0.5
        onset_rate = float(features.get('onset_rate_norm', 0.5)) if features.get('onset_rate_norm') is not None else 0.5
        rhythm_regularity = float(features.get('rhythm_regularity', 0.5)) if features.get('rhythm_regularity') is not None else 0.5
        spectral_bandwidth = float(features.get('spectral_bandwidth_norm', 0.5)) if features.get('spectral_bandwidth_norm') is not None else 0.5
        tempo_stability = float(features.get('tempo_stability', 0.5)) if features.get('tempo_stability') is not None else 0.5
        
        # Get MFCC features for timbre analysis
        mfcc_energy = float(features.get('mfcc_energy', 0.0)) if features.get('mfcc_energy') is not None else 0.0
        ...
        mode_numeric = 1.0 if mode == 'major' else (0.0 if mode == 'minor' else 0.5)
        ...
        if tempo >= 120 and percussive_ratio > 0.35 and rhythm_regularity > 0.55:
            ...
            electronic_score = (
                ...
                onset_rate * 0.15 +
                (spectral_centroid * 0.8 + 0.2) * 0.15  # Prefer moderate-high brightness
            )
        ...
        if 95 < tempo < 165:
            ...
            rock_score = (
                ...
                (1.0 - abs(spectral_centroid - 0.5) / 0.5) * 0.25 +  # Balanced brightness
                harmonic_ratio * 0.20 +
                rhythm_regularity * 0.15 +
                (1.0 - abs(percussive_ratio - 0.5) / 0.5) * 0.10  # Balanced percussive/harmonic
            )
        ...
        if harmonic_ratio > 0.55:
            jazz_score = (
                harmonic_ratio * 0.35 +
                (1.0 - rhythm_regularity * 0.8) * 0.25 +  # Less regular = more jazz-like
        ...
        if 95 < tempo < 145:
            pop_score = (
                ...
                (1.0 - abs(harmonic_ratio - 0.6) / 0.6) * 0.25 +  # Balanced harmonic
        ...
        if 75 < tempo < 125:
            blues_score = (
                ...
                (1.0 - abs(rhythm_regularity - 0.5) / 0.5) * 0.20  # Moderate regularity
            )
...
def classify_sub_genre(features: FeatureDict, dominant_genre: str) -> Dict[str, float]:
    """
        ...
        # Rock sub-genres
            ...
            if 110 < tempo < 150:
                sub_genre_scores['alternative_rock'] = (
                    ...
                    (1.0 - rhythm_regularity * 0.9) * 0.3  # Slightly less regular
                )
        ...
        # Hip-hop sub-genres
            ...
            if 75 < tempo < 95:
                sub_genre_scores['old_school'] = (
                    (1.0 - abs(tempo - 85) / 10) * 0.5 +
                    (1.0 - abs(harmonic_ratio - 0.5) / 0.5) * 0.5
                )
...
def create_librosa_extractor(
    ...
    if update_interval <= 0:
        raise ValueError(f"Invalid update_interval: {update_interval} (must be > 0)")
    
    try:
        buffer = LibrosaAudioBuffer(sample_rate, buffer_duration)
        extractor = LibrosaFeatureExtractor(buffer, sample_rate, update_interval)
        _LOGGER.debug(
        ...
        _LOGGER.warning(f"Failed to create librosa extractor: {e}", exc_info=True)
        return None
