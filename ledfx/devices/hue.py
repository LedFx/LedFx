...
class HueDevice(NetworkedDevice):
    ...
    def _hue_register(self):
        if (self._config.get("username") is None) and (self._config.get("clientkey") is None):
            # We need to register this device as application at the Hue Bridge.
            request_data = {"devicetype": f"LedFx#{self._config['group_name']}", "generateclientkey": True}
            ...
            if "success" in response[0]:
                ...
                raise Exception("You need to press the Bridge Link Button and retry that again.")
        else:
            ...
            if "error" in response[0]:
                ...
                raise Exception("You need to press the Bridge Link Button and restart LedFx.")

    def _check_hue_bridge(self):
        response, _ = self._hue_request("GET", "api/config")
        if response["swversion"] < "1948086000":
            raise Exception("Your Hue Bridge has an outdated Firmware installed. Update it using the Hue App.")
    ...
    def _hue_request(self, method, api_endpoint, data=None, ssl=False):
        ...
        headers = {
            'hue-application-key': self._config.get("username")
        }
    ...
    def _entertainment_groups(self):
        response, _ = self._hue_request("GET", "/clip/v2/resource/entertainment_configuration", ssl=True)

        all_groups = response["data"]
        entertainmentZonesCount = len(all_groups)

        if entertainmentZonesCount == 0:
            raise Exception("You did not setup any Entertainment zones. Do that in the Hue App.")
        
        
        return { group['id']: group for group in all_groups }

    def _lights_from_entertainment_group(self, entertainment_id):
        response, _ = self._hue_request("GET", f"/clip/v2/resource/entertainment_configuration/{entertainment_id}", ssl=True)
        lights = dict()
        for channel in response['data'][0]['channels']:
            lights.update({str(channel["channel_id"]): [channel['position']['x'], channel['position']['y'], channel['position']['z']]})

        if len(lights) > 20:
            raise Exception(f"{len(lights)} lights found. Only 20 are allowed.")
    ...
    def activate(self):
        ...
        self._hue_request("PUT", f"/clip/v2/resource/entertainment_configuration/{self._config['entertainment_id']}", request_data, ssl=True)

        sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        sock.settimeout(5)
        sock.setblocking(False)
        self._sock = self._dtls_client_context.wrap_socket(sock, self._config["ip_address"])
        self._sock.connect(
        ...
        for _ in range(10):
            try:
                ...
                _LOGGER.warning(f"Failed to establish TLS handshake when activating the UDP stream. Retrying. {e}")

        if not handshake_success:
            raise Exception("Could not connect to the Bridge. Disconnect and reconnect it from power.")
    ...
    def deactivate(self):
        ...
        response, _ = self._hue_request("PUT", f"/clip/v2/resource/entertainment_configuration/{self._config['entertainment_id']}", request_data, ssl=True)
