    ...
    def LedCtrlString(self, text, red, green, direction=None, waitms=150):
        ...
        if direction == self.SCROLL_LEFT:
            ...
            for n in range((len(text) + 1) * 8):
                if n <= len(text) * 8:
                    self.LedCtrlChar(
                        text[limit((n // 16) * 2, 0, len(text) - 1)],
                        red,
                        green,
                        8 - n % 16,
                    )
...
class LaunchpadPro(LaunchpadBase):
    ...
    # -------------------------------------------------------------------------------------
    def LedCtrlString(
        self, text, red, green, blue=None, direction=None, waitms=150
    ):
        # compatibility mode
        if blue is None:
            red *= 21
...
class Dicer(LaunchpadBase):
    ...
    def LedSetLightshow(self, device, enable):
        ...
        self.midi.RawWrite(
            186 if device == 0 else 189, 0, 40 if enable == True else 41
        )
...
class LaunchpadLPX(LaunchpadPro):
    ...
    def LedCtrlRaw(self, number, red, green, blue=None):
        ...
        self.midi.RawWriteSysEx(
            [0, 32, 41, 2, 12, 3, 3, number, red, green, blue]
        )
...
class MidiFighter64(LaunchpadBase):
    ...
    # -------------------------------------------------------------------------------------
    def LedCtrlString(
        self, text, colorcode, coloroff=0, direction=None, waitms=150
    ):
        limit = lambda n, mini, maxi: max(min(maxi, n), mini)
...
class LaunchpadProMk3(LaunchpadPro):
    ...
    def LedCtrlRaw(self, number, red, green, blue=None):
        ...
        self.midi.RawWriteSysEx(
            [0, 32, 41, 2, 14, 3, 3, number, red, green, blue]
        )
