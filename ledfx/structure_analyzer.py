...
class MusicStructureAnalyzer:
    ...
    def __init__(
        self, audio_source, mood_detector, config: Optional[dict] = None
    ):
        """
    ...
    def _detect_event(
        self, mood: dict[str, float], current_time: float
        ...
        dominant_genre = (
            mood.get("dominant_style", "").lower()
            if mood.get("dominant_style")
            else ""
        )
        genre_confidence = (
            float(mood.get("style_confidence", 0.0))
            if mood.get("style_confidence")
            else 0.0
        )
        ...
        if (
            genre_confidence > 0.2
        ):  # Only adjust if we have reasonable genre confidence
        ...
        # Enhanced detection with genre-aware thresholds
            ...
            if (
                energy_spike > adjusted_drop_threshold
                and beat_strength > beat_requirement
            ):
        ...
        # Enhanced with genre-aware thresholds and better validation
            ...
            self._energy_trend > (build_threshold * (1.0 + sensitivity))
            and not self._in_build
        ):
            ...
            if dominant_genre in ["electronic", "metal", "rock"]:
                intensity_requirement = (
                    0.4  # More lenient for high-energy genres
                )
            elif dominant_genre in ["jazz", "classical", "ambient"]:
                intensity_requirement = (
                    0.6  # Require more intensity for subtle genres
                )
        ...
        # Enhanced with genre-aware timing
            ...
            if (
                time_building > min_build_time
                and energy_gain > min_energy_gain
            ):
                ...
                # Or if energy starts dropping after peak
                    ...
                    self._energy_trend < -0.1
                    and energy > self._peak_energy * 0.95
                ):
                    event = StructuralEvent.CLIMAX
                    self._in_build = False

        # Detect sustained energy rise (genre-aware)
        elif self._energy_trend > (
            energy_change_threshold * (1.0 + sensitivity)
        ):
            if self._last_event != StructuralEvent.ENERGY_RISE:
                ...
                if (
                    time_since_last
                    > self._config["event_cooldown"] * cooldown_multiplier
                ):
        ...
        # Enhanced with genre-aware thresholds
            ...
            spectral_drop = np.mean(recent_spectral[:5]) - np.mean(
                recent_spectral[5:]
            )

            # Genre-specific breakdown detection
            adjusted_breakdown_threshold = breakdown_threshold * (
                1.0 + sensitivity
            )
            ...
            if (
                spectral_drop > adjusted_breakdown_threshold
                and energy < energy_requirement
            ):
        ...
        if event is not None:
            ...
            genre_info = (
                f" (genre: {dominant_genre})" if dominant_genre else ""
            )
            _LOGGER.info(
                f"Detected structural event: {event.value}{genre_info}"
            )

        return event

    def _update_section(
        self, mood: dict[str, float], event: Optional[StructuralEvent]
    ):
        """
        ...
        # Classify based on energy and intensity
            ...
            # Verse: moderate energy, structured
                ...
                if self._current_section not in [
                    MusicSection.VERSE,
