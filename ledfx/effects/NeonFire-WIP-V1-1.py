...
class NeonFire_WIP_V1_1(Twod):
    ...
    def do_once(self):
        ...
        self.imagebuffer = Image.new(
            "RGBA", (self.r_width, self.r_height), (0, 0, 0, 0)
        )
    ...
    def prep_frame_vars(self):
        ...
        if len(out_split) >= self.r_height:
            for i in range(self.r_height):
                vol = out_split[i].max()
                pixelcolor = (
                    int(self.color[0] * vol),
                    int(self.color[1] * vol),
                    int(self.color[2] * vol),
                    255,
                )
                ...
                if self.hsvcolor:
                    ...
                    if self.peaksense > 0:
                        peakresult = int(
                            255 / self.peaksense
                            + out_split[i].mean() * self.peaksense * 255
                        )
                    else:
                        peakresult = 255
                    pixelcolor = (
    ...
    def modifybuffer(self):
        ...
        if self.mirroring:
            reversebuffer = self.pastebuffer.transpose(
                method=Image.Transpose.FLIP_LEFT_RIGHT
            )
            # self.pastebuffer = Image.alpha_composite(self.pastebuffer, reversebuffer)
            self.pastebuffer = Image.composite(
                self.pastebuffer, reversebuffer, self.pastebuffer
            )
