...
class GameOfLifeVisualiser(Twod, GradientEffect):
    ...
    def __init__(self, ledfx, config):
        ...
        self.last_game_step = self.start_time
    ...
    def do_once(self):
        super().do_once()
        self.game = GameOfLife(height=self.r_height, width=self.r_width)
    ...
    def draw(self):
        ...
        self.check_board_health(current_time)
        self.step_board_if_time_elapsed(current_time)
        self.update_image_with_board(self.matrix)
    ...
    def update_image_with_board(self, image):
        """
        Updates the image with the current game board.

        Args:
            image (PIL.Image.Image): The original image object.

        """
        np_image = np.array(image)
        mask = np.uint8(self.game.board)
        np_image[mask == 1] = [255, 255, 255]
        updated_image = Image.fromarray(np_image)
        image.paste(updated_image, (0, 0))
...
class GameOfLife:
    ...
    def check_board_oscillating(self, lookback_generations=10):
        """
